...
function openProduct(id=null){
  ...
  <div class="form-grid">
  <label>Product name<input id="pName" class="input" value="${esc(p?.name||"")}"></label>
  <label>SKU<input id="pSku" class="input" value="${esc(p?.sku||"")}"></label>
  <label>Category<input id="pCat" class="input" value="${esc(p?.category||"Solar / Electrical")}"></label>
  ...
  $("#saveProductBtn").onclick=()=>{
    const obj={id:p?.id||uid("prod"),name:$("#pName").value.trim(),sku:$("#pSku").value.trim(),category:$("#pCat").value.trim(),qty:+$("#pQty").value,minQty:+$("#pMin").value,cost:+$("#pCost").value,price:+$("#pPrice").value};
    if(!obj.name){toast("Product name is required.");return}
    mutate(()=>{if(p)Object.assign(p,obj);else state.products.push(obj)});closeModal();
  };
}
